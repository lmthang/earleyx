...
//    assertEquals(true, compare(parser.getMeasureList(Measures.ENTROPY_REDUCTION), new double[]{0.7515343091896578, 0.5101419650652941, 0.8316358385027407, 0.6460702812892124}));
  ...
  public void testExtendedGrammarIO(){
    ...
    if(insideOutsideOpt>0){
      ...
      try {
        objectiveList = io.insideOutside(inputSentences, "", 1e-20f);
      } catch (IOException e) {
      ...
      if(insideOutsideOpt==1){
        ...
        double[] results = new double[]{2.5341702140023417, 2.1016008502172006, 2.0945469475248073, 2.0930014040440703, 2.0926627204076063, 2.0925882495532324, 2.092571750466667, 2.0925680371884927, 2.092567174802319, 2.092566962421006, 2.092566904801532};
  ...
  public void testMarkGrammar(){
    ...
    if (isLogProb){
    	assertEquals(true, compare(parser.dumpInsideChart(), "# Inside chart snapshot\ncell 0-1\n Det: 0.1428571\n N: 0.1428571\n V: 0.1428571\ncell 1-2\n Det: 0.1428571\n N: 0.1428571\n V: 0.1428571\ncell 2-3\n Det: 0.1428571\n N: 0.1428571\n V: 0.1428571\ncell 3-4\n Det: 0.1428571\n N: 0.1428571\n V: 0.1428571\ncell 4-5\n Det: 0.1428571\n N: 0.1428571\n V: 0.1428571\ncell 0-2\n NP: 0.020408152\ncell 2-4\n NP: 0.020408152\ncell 3-5\n NP: 0.020408152\ncell 0-3\n : 5.8308983E-4\n S: 5.8308983E-4\ncell 2-5\n VP: 0.0011661802\ncell 0-5\n : 2.3799581E-5\n S: 2.3799581E-5\n"));
    } else {
      if(isScaling){
    ...
    if(insideOutsideOpt>0){
      ...
      assertEquals(true, compare(parser.sprintExpectedCounts(), "# Expected counts\n6.000000 S->[NP VP]\n6.500000 NP->[Det N]\n6.500000 NP->[N Det]\n1.000000 VP->[V]\n1.500000 VP->[V NP]\n1.500000 VP->[NP V]\n1.000000 VP->[V NP NP]\n1.000000 VP->[NP NP V]\n3.000000 Det->[_the]\n3.000000 N->[_the]\n3.500000 Det->[_a]\n3.500000 N->[_a]\n2.750000 Det->[_dog]\n2.750000 N->[_dog]\n0.500000 V->[_dog]\n1.750000 Det->[_cat]\n1.750000 N->[_cat]\n0.500000 V->[_cat]\n0.750000 Det->[_bone]\n0.750000 N->[_bone]\n1.500000 V->[_bone]\n0.750000 Det->[_bites]\n0.750000 N->[_bites]\n2.500000 V->[_bites]\n0.500000 Det->[_gives]\n0.500000 N->[_gives]\n1.000000 V->[_gives]\n"));
    }    
  }
    ...
    if(insideOutsideOpt>0){
      ...
      if(insideOutsideOpt==1){
        ...
        assertEquals(true, compare(objectiveList, new double[]{68.46002594157635, 58.51055584306548, 55.220924477124214, 53.7010153144059, 52.26369575985821, 50.759354005056494, 50.63455865171508, 50.63452160196377, 50.63452160196377}));
      	...
      	assertEquals(true, compare(parser.sprintExpectedCounts(), "# Expected counts\n6.000000 S->[NP VP]\n12.999637 NP->[Det N]\n0.000363 NP->[N Det]\n1.000000 VP->[V]\n2.999692 VP->[V NP]\n0.000308 VP->[NP V]\n1.999972 VP->[V NP NP]\n0.000028 VP->[NP NP V]\n6.000000 Det->[_the]\n7.000000 Det->[_a]\n5.999928 N->[_dog]\n0.000072 V->[_dog]\n3.999897 N->[_cat]\n0.000103 V->[_cat]\n2.999840 N->[_bone]\n0.000160 V->[_bone]\n0.000308 N->[_bites]\n3.999692 V->[_bites]\n0.000028 N->[_gives]\n1.999972 V->[_gives]\n"));  
        assertEquals(true, compare(objectiveList, new double[]{92.93296417270264, 89.56984599647323, 81.96438670486147, 75.03608617145746, 61.47488814240124, 55.81162717434088, 54.26289146637768, 52.8466446613439, 52.8466444968588}));
      }
...
//    inputSentences.add("the dog bites");
    ...
    if(insideOutsideOpt>0){
      ...
      if(insideOutsideOpt==1){
        ...
        double[] results = new double[]{76.92231224909392, 50.90073342745999, 50.87643752798018, 50.87351774507555};
        assertEquals(true, compare(objectiveList, results));
      }
    }
  }
  ...
  /**
   * Robust comparison in the sense that the texts are equal
   * if numbers appearing in the text differ only by a small amount.
   * @param text1
   * @param text2
   * @return
   */
  private boolean compare(String text1, String text2){
    ...
    if(tokens1.length == tokens2.length){
      for (int i = 0; i < tokens2.length; i++) {
      	boolean flag1 = isNumber(tokens1[i]);
      	boolean flag2 = isNumber(tokens2[i]);
      	if(flag1 != flag2){
      		System.err.println("Type differ: " + tokens1[i] + "\t" + tokens2[i]);
        	isEqual = false;
        	break;
      	} else if (flag1) { // number
      		double num1 = Double.parseDouble(tokens1[i]);
      		double num2 = Double.parseDouble(tokens2[i]);
      		if(Math.abs(num1-num2)>1e-5){
      			System.err.println("Value differ: " + tokens1[i] + "\t" + tokens2[i]);
          	isEqual = false;
          	break;
      		}
      	} else if (!tokens1[i].equals(tokens2[i])){
      		System.err.println("Value differ: " + tokens1[i] + "\t" + tokens2[i]);
        	isEqual = false;
      	}
      }
    } else {
    	System.err.println("Lengths differ: " + tokens1.length + "\t" + tokens2.length);
    	isEqual = false;
    }
    ...
    if(values.size() == results.length){
      for (int i = 0; i < results.length; i++) {
        if(Math.abs(values.get(i)-results[i])>1e-5){
        	System.err.println("Values differ: " + values.get(i) + "\t" + results[i]);
        	isEqual = false;
        	break;
        }
      }
    } else {
    	System.err.println("Lengths differ: " + values.size() + "\t" + results.length);
    	isEqual = false;
    }
  ...
  public void testSocialUnigramPseudoIO(){
    ...
    if(insideOutsideOpt>0){
      ...
      if(insideOutsideOpt==1){
        ...
        assertEquals(true, compare(objectiveList, new double[]{119.57753371025353, 56.25527212925164, 55.55705760873095, 50.09255118022883, 49.947780601385176, 49.94772549410479, 49.947725459941054}));
      }
    }
  }
  ...
  public void testSocialUnigramNonPseudoIO(){
    ...
    if(insideOutsideOpt>0){
      ...
      if(insideOutsideOpt==1){
        ...
        double[] results = new double[]{119.57753371025353, 56.25527212925161, 55.55705760873092, 50.092551180228845, 49.947780601385176, 49.947725494104716, 49.947725459941026};
        assertEquals(true, compare(objectiveList, results));
      }
    }
  }
...
//  }
  ...
  public void testLeftInfiniteGrammar(){
    ...
    assertEquals(0, surprisalList.get(0), 1e-6);
    
    
    assert(stringProbList.size()==numSymbols);
    assertEquals(p, stringProbList.get(0), 1e-6);
    
    for (int i = 1; i < numSymbols; i++) {
      // strings of (i+1) x: string prob = p*q^i, prefix prob = q^i, surprisal = -log(q)
      assertEquals(-Math.log(q), surprisalList.get(i), 1e-6);
      if(!isScaling){
        assertEquals(p*Math.pow(q, i), stringProbList.get(i), 1e-6);
      }
    }
  }
...
//    System.err.println(parser.dumpOutsideChart());
  ...
  public void testCatalanGrammar(){
    ...
    for (int i = 1; i < numSymbols; i++) {
      ...
      assertEquals(currentStringProb, stringProbList.get(i), 1e-6);
      
      double currentPrefixProb = prevPrefixProb - prevStringProb;
      assertEquals(-Math.log(currentPrefixProb/prevPrefixProb), surprisalList.get(i), 1e-6);
...
//    }
    ...
    assertEquals(true, compare(stringProbList,  new double[]{0.0, 0.405}));
...
//    }
    ...
    List<Double> stringProbList = parser.getMeasureList(Measures.STRINGPROB);
    assertEquals(true, compare(stringProbList, new double[]{0.0, 0.04000000000000001}));
...
//    }
  ...
  public void testWSJ500AG(){
    ...
    assertEquals(true, compare(parser.getMeasureList(Measures.SURPRISAL), new double[]{2.5902917235748943, 11.178023682698445, 6.67950724000549, 7.077894219399997, 7.981383181401906, 6.445714413247863, 4.424279317015973, 7.537896375777983, 7.137751990560567, 3.413081532602021, 7.724491276999544, 7.288287721987004, 3.2146524173861195}));
